...
class TaskCalendar {
    constructor() {
        ...
        this.taskDetailsModal = new Modal(
            document.getElementById("task-details-modal")
        );
        this.taskFormModal = new Modal(
    ...
    setupTaskFormModal() {
        ...
        if (isStaff) {
            const modalQuickEditBtn = document.getElementById(
                "modal-quick-edit-btn"
            );
            if (modalQuickEditBtn) {
                modalQuickEditBtn.style.display = "none";
            }
    ...
    openTaskFormModal(date = null) {
        ...
        document.getElementById("taskFormModalLabel").textContent =
            "Add New Task";
    ...
    openTaskFormModalForEdit() {
        ...
        taskDate.setDate(taskDate.getDate() + 1); // Add 1 day
        const formattedDate = taskDate.toISOString().split("T")[0];
        document.getElementById("modal-customer_id").value =
            props.customer_id || "";
        document.getElementById("modal-task_date").value = formattedDate;
        document.getElementById("modal-task_type_id").value =
            props.task_type_id || "";
        document.getElementById("modal-assigned_to").value =
            props.assigned_to || "";
        document.getElementById("modal-status").value =
            props.status || "pending";
        document.getElementById("modal-publish-status").value =
            props.publish_status ?? null;
        document.getElementById("modal-note_content").value =
            props.note_content || "";

        const PUBLISHABLE_TASK_TYPES = ["Graphic Design", "Video Editing"];
        console.log(props);
        if (PUBLISHABLE_TASK_TYPES.includes(props.task_type)) {
    ...
    submitTaskForm() {
        ...
        submitBtn.innerHTML =
            '<i class="bx bx-loader bx-spin me-1"></i> Saving...';
    ...
    showTaskDetails(event) {
        ...
        document.getElementById(
            "modal-status"
        ).innerHTML = `<span class="badge bg-${this.getStatusColor(
            props.status
        )}">${props.status_label}</span>`;
        ...
        document.getElementById("modal-notes").textContent =
            props.note_content || "No notes available";

        if (!isStaff) {
            // Set edit link with calendar redirect
            document.getElementById(
                "modal-edit-link"
            ).href = `/tasks/${props.task_id}/edit?redirect_to=calendar`;
        }
    ...
    updateTaskStatus() {
        ...
        submitButton.innerHTML =
            '<i class="bx bx-loader bx-spin me-1"></i> Updating...';
        ...
        fetch(`/tasks/${this.currentTaskId}/update-status`, {
            ...
            .then((data) => {
                if (data.success) {
                    ...
                    statusDisplay.innerHTML = `<span class="badge bg-${this.getStatusColor(
                        newStatus
                    )}">${statusLabel}</span>`;
    ...
    updateStaffOptions(staffWorkload) {
        ...
        options.forEach((option) => {
            ...
            if (staffData) {
                const capacityBadge = this.getCapacityBadge(
                    staffData.capacity_percentage
                );
                const availabilityText = staffData.can_assign_more
                    ? "Available"
                    : "At Capacity";
